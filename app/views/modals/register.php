  ...
  >
    <div class="pointer-events-auto relative flex flex-col rounded-md bg-white p-6 shadow-lg outline-none">
      <div class="flex items-center justify-between border-b pb-3">
        <h5 class="text-xl font-medium leading-normal">Регистрация</h5>
        <button
          type="button"
      ...
      <form method="POST" onsubmit="handleRegister(event)" class="mt-4 space-y-4" id="registerForm">
        <div id="registerErrors" class="hidden bg-red-100 border border-red-400 text-red-700 px-4 py-2 rounded text-sm"></div>

        <div>
          <label for="reg_login" class="block text-sm font-medium mb-1">Логин</label>
        ...
        <div>
          <label for="reg_password" class="block text-sm font-medium mb-1">Пароль</label>
          <input type="password" name="password" id="reg_password" required
                class="w-full px-4 py-2 border rounded focus:outline-none focus:ring focus:border-blue-500" />
        </div>

        <div>
          <label for="reg_confirm" class="block text-sm font-medium mb-1">Повтор пароля</label>
